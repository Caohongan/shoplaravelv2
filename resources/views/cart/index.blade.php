          ...
          <td class="product_info">
            ...
            <div @if(!$item->productSku->product->on_sale) class="not_on_sale" @endif>
              ...
              @if(!$item->productSku->product->on_sale)
                <span class="warning">Sản phẩm này đã được gỡ bỏ</span>
              @endif
            </div>
          </td>
    ...
    <div>
      <form class="form-horizontal" role="form" id="order-form">
        <div class="form-group row">
          <label class="col-form-label col-sm-3 text-md-right">Chọn địa chỉ giao hàng</label>
        ...
        <div class="form-group row">
          <label class="col-form-label col-sm-3 text-md-right">Ghi chú</label>
          <div class="col-sm-9 col-md-7">
        ...
        <!-- 优惠码开始 -->
        <div class="form-group row">
          <label class="col-form-label col-sm-3 text-md-right">Mã giảm giá</label>
          <div class="col-sm-4">
            <input type="text" class="form-control" name="coupon_code">
            <span class="form-text text-muted" id="coupon_desc"></span>
          </div>
          <div class="col-sm-3">
            <button type="button" class="btn btn-success" id="btn-check-coupon">Kiểm tra</button>
            <button type="button" class="btn btn-danger" style="display: none;" id="btn-cancel-coupon">Hủy bỏ</button>
          </div>
        </div>
        <!-- 优惠码结束 -->
        <div class="form-group">
          <div class="offset-sm-3 col-sm-3">
            <button type="button" class="btn btn-primary btn-create-order">Gửi đơn đặt hàng</button>
          </div>
        </div>
      </form>
    </div>
  ...
  <script>
    $(document).ready(function () {
      ...
      $('.btn-remove').click(function () {
        ...
        swal({
          title: "Bạn có chắc muốn bỏ không ?",
          icon: "warning",
          buttons: ['No', 'Yes'],
          dangerMode: true,
        })
        ...
        axios.post('{{ route('orders.store') }}', req)
          .then(function (response) {
            ...
            if (error.response.status === 422) {
              ...
              html += '</div>';
              swal('Không có sản phẩm hoặc sản phẩm này đã hết.', '', 'error')
      ...
      $('#btn-check-coupon').click(function () {
        ...
        // 如果没有输入则弹框提示
        if(!code) {
          swal('Vui lòng nhập mã giảm giá', '', 'warning');
          return;
        }
        // 调用检查接口
        axios.get('/coupon_codes/' + encodeURIComponent(code))
          .then(function (response) {  // then 方法的第一个参数是回调，请求成功时会被调用
            ...
            if(error.response.status === 404) {
              swal('Mã phiếu giảm giá không tồn tại', '', 'error');
