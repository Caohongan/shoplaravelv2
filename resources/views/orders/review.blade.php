...
<div class="card">
  ...
  Đánh giá sản phẩm
    <a class="float-right" href="{{ route('orders.index') }}">Quay lại danh sách đặt hàng</a>
  </div>
  <div class="card-body">
      ...
      <table class="table">
        ...
        @foreach($order->items as $index => $item)
          <tr>
            ...
            <td class="vertical-middle">
              <!-- 如果订单已经 field is required.价则展示评分，下同 -->
              @if($order->reviewed)
            ...
            <td>
              @if($order->reviewed)
                {{ $item->review }}
              @else
                <textarea required class=" form-control {{ $errors->has('reviews.'.$index.'.review') ? 'is-invalid' : '' }}" name="reviews[{{$index}}][review]"></textarea>
        ...
        <tr>
          <td colspan="3" class="text-center">
            @if(!$order->reviewed)
              <button type="submit" class="btn btn-primary center-block">Gửi</button>
            @else
              <a href="{{ route('orders.show', [$order->id]) }}" class="btn btn-primary">Xem thứ tự</a>
            @endif
          </td>
        </tr>
        </tfoot>
